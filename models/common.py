...
class IV6Detect(nn.Module):
    def __init__(self, nc=80, ch=(), strides=(), reg_max=16, training=True):
    ...
    def forward(self, x):
        ...
        self.anchors, self.strides_grid = (x.transpose(0, 1) for x in make_anchors(x, self.strides, 0.5))
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides_grid
        y = torch.cat((dbox, cls.sigmoid()), 1)
        return y

    def bias_init(self):
        # Initialize Detect() biases, WARNING: requires stride availability
        print('initial bias of output')
        for a, b, s in zip(self.cv2, self.cv3, self.strides):  # from
            a[-1].bias.data[:] = 1.0  # box
            b[-1].bias.data[:self.nc] = math.log(5 / self.nc / (640 / s) ** 2)  # cls (5 objects and 80 classes per 640 image)